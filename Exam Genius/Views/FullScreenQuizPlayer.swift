...
//                                    .blur(radius: 100)
                    ...
                    .matchedGeometryEffect(id: user.selectedQuizPackage?.imageUrl.uppercased() ?? "ICONIMAGE", in: animation)
...
//                                endAction: { quizPlayer.resetForNextQuiz()})
        ...
        .onAppear {
            generator.updateAllColors(fromImageNamed: user.selectedQuizPackage?.imageUrl ?? "IconImage")
...
#Preview {
    ...
    return FullScreenQuizPlayer(expandSheet: .constant(false), quizPlayer: quizPlayer, animation: animation)
